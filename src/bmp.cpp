...
void Bmp::load(const char *fileName) {
  ...
  // le o HEADER componente a componente devido ao problema de alinhamento de
  // bytes. Usando o comando fread(header, sizeof(HEADER),1,fp) sao lidos 16
  // bytes ao inves de 14
  ...
  fread(&header.offset, sizeof(unsigned int), 1,
        fp); // indica inicio do bloco de pixels

  // le o INFOHEADER componente a componente devido ao problema de alinhamento
  // de bytes
...
void Bmp::render(Bmp *img, unsigned char *data) {
  for (float y = 0; y < img->getHeight(); y++) {
    for (float x = 0; x < img->width * 3; x += 3) {
      int pos = y * img->bytesPerLine + x;

      printf("\n%d %d %d", data[pos], data[pos + 1], data[pos + 2]);
      float r = float(data[pos]) / 255;
      float g = float(data[pos + 1]) / 255;
      float b = float(data[pos + 2]) / 255;
