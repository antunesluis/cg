...
class FPSController
{
  private:
    ...
    const float MAX_DELTA_TIME = 0.1f; // Limite para o deltaTime para evitar problemas em baixos FPS
  ...
  public:
    ...
    void update()
    {
        ...
        m_frameCount++;
        auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - m_lastFPSUpdate).count();
