...
void ConvertMouseCoord(int button, int state, int wheel, int direction, int x,
                       int y);
...
void CV::circle(float x, float y, float radius, int div) {
  ...
  for (int lado = 1; lado <= div;
       lado++) // GL_LINE_LOOP desenha um poligono fechado. Liga automaticamente
...
void CV::circleFill(float x, float y, float radius, int div) {
  ...
  glBegin(GL_POLYGON);
  for (int lado = 1; lado <= div;
       lado++) // GL_POLYGON desenha um poligono CONVEXO preenchido.
  {
    x1 = (cos(ang) * radius);
    y1 = (sin(ang) * radius);
    glVertex2d(x1 + x, y1 + y);
...
void ConvertMouseCoord(int button, int state, int wheel, int direction, int x,
  ...
  y = *scrHeight -
      y; // deve-se inverter a coordenada y do mouse se o y da canvas crescer
...
void reshape(int w, int h) {
  *scrHeight =
      h; // atualiza as variaveis da main() com a nova dimensao da tela.
  *scrWidth = w;
