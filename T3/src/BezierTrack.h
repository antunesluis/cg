...
class BezierTrack
{
  ...
  public:
    ...
    CollisionResult checkCollision(const Vector2 &position, float radius) const
    {
        ...
        auto checkBoundary = [&](const std::vector<Vector2> &boundary) {
            for (size_t i = 0; i < boundary.size() - 1; i++) {
                Vector2 closest = closestPointOnSegment(position, boundary[i], boundary[i + 1]);
                ...
                if (dist < radius) {
                    ...
                    if (penetration > result.penetrationDepth) {
                        ...
                        if (normal.length() > 0.001f) { // Assegura que a normal não é um vetor nulo
                            normal.normalize();
    ...
    Vector2 closestPointOnSegment(const Vector2 &p, const Vector2 &a, const Vector2 &b) const
    {
        Vector2 ab = b - a;
    ...
    bool isInsideTrack(const Vector2 &point) const
    {
        ...
        for (size_t i = 0; i < innerBoundary.size() - 1; i++) {
            float dist = distanceToLineSegment(point, innerBoundary[i], innerBoundary[i + 1]);
            minDistInner = std::min(minDistInner, dist);
        }

        for (size_t i = 0; i < outerBoundary.size() - 1; i++) {
            float dist = distanceToLineSegment(point, outerBoundary[i], outerBoundary[i + 1]);
    ...
    float distanceToLineSegment(const Vector2 &point, const Vector2 &lineStart, const Vector2 &lineEnd) const
    {
        ...
        for (size_t i = 0; i < trackPoints.size() - 1; i++) {
            ...
            while (segmentPos < segmentLength) {
                ...
                if (drawing) {
                    ...
                    float dashLength = std::min(centerLineDashLength - dashStartPos, remainingInSegment);
                    ...
                    float vx[4] = {dashStart.x + perpendicular.x, dashStart.x - perpendicular.x,
                                   dashEnd.x - perpendicular.x, dashEnd.x + perpendicular.x};
                    float vy[4] = {dashStart.y + perpendicular.y, dashStart.y - perpendicular.y,
                    ...
                    float gapLength = std::min(centerLineGapLength - dashStartPos, remainingInSegment);
    ...
    void renderTrackSurface()
    {
        ...
        for (size_t i = 0; i < innerBoundary.size() - 1 && i < outerBoundary.size() - 1; i++) {
            float vx[4] = {innerBoundary[i].x, innerBoundary[i + 1].x, outerBoundary[i + 1].x, outerBoundary[i].x};
            float vy[4] = {innerBoundary[i].y, innerBoundary[i + 1].y, outerBoundary[i + 1].y, outerBoundary[i].y};
        ...
        if (closeTrack && innerBoundary.size() > 2 && outerBoundary.size() > 2) {
            float vx[4] = {innerBoundary[innerBoundary.size() - 1].x, innerBoundary[0].x, outerBoundary[0].x,
                           outerBoundary[outerBoundary.size() - 1].x};
            float vy[4] = {innerBoundary[innerBoundary.size() - 1].y, innerBoundary[0].y, outerBoundary[0].y,
    ...
    void render()
    {
        ...
        if (innerBoundary.size() > 1 && outerBoundary.size() > 1) {
            ...
            for (size_t i = 0; i < innerBoundary.size() - 1; i++) {
                CV::line(innerBoundary[i].x, innerBoundary[i].y, innerBoundary[i + 1].x, innerBoundary[i + 1].y);
            }

            // Fecha o limite interno se necessário
            if (closeTrack && innerBoundary.size() > 1) {
                CV::line(innerBoundary[innerBoundary.size() - 1].x, innerBoundary[innerBoundary.size() - 1].y,
                         innerBoundary[0].x, innerBoundary[0].y);
            }

            // Limite externo
            Colors::roadEdgeMarking();
            for (size_t i = 0; i < outerBoundary.size() - 1; i++) {
                CV::line(outerBoundary[i].x, outerBoundary[i].y, outerBoundary[i + 1].x, outerBoundary[i + 1].y);
            }

            // Fecha o limite externo se necessário
            if (closeTrack && outerBoundary.size() > 1) {
                CV::line(outerBoundary[outerBoundary.size() - 1].x, outerBoundary[outerBoundary.size() - 1].y,
                         outerBoundary[0].x, outerBoundary[0].y);
            }
        }
        ...
        if (editMode) {
            ...
            if (controlPoints.size() >= 2) {
                ...
                if (closeTrack && controlPoints.size() >= 3) {
                    CV::line(controlPoints[controlPoints.size() - 1].x, controlPoints[controlPoints.size() - 1].y,
