...
Slider::Slider(float x, float y, float w, float h, float minVal, float maxVal,
               float initialVal, const std::string &lbl,
               std::function<void(float)> callback)
    : UIElement(x, y, w, h, lbl), min(minVal), max(maxVal), value(initialVal),
      label(lbl), onChangeCallback(callback), sliderWidth(w - 40),
      dragging(false) {}
...
void Slider::render() const {
  ...
  CV::rectFill(x + 20, y + height / 2 - 5, x + 20 + sliderWidth,
               y + height / 2 + 5);
  ...
  CV::text(x + width - 35, y + height / 2 - 10,
           std::to_string(value).substr(0, 4).c_str());
}
...
bool Slider::checkInteraction(int mouseX, int mouseY, int buttonState) {
  ...
  if (buttonState == 0) { // Botão pressionado
    if (mouseX >= x + 15 && mouseX <= x + 25 + sliderWidth && mouseY >= y &&
        mouseY <= y + height) {
      dragging = true;
    }
