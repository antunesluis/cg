...
void Checkbox::render() const {
  // Caixa do checkbox
  CV::color(config::colors::checkbox[0], config::colors::checkbox[1],
            config::colors::checkbox[2]);
  CV::rectFill(x, y, x + boxSize, y + boxSize);

  // Borda
  CV::color(config::colors::checkbox_border[0],
            config::colors::checkbox_border[1],
            config::colors::checkbox_border[2]);
  CV::rect(x, y, x + boxSize, y + boxSize);

  // Checkmark
  if (checked) {
    CV::color(config::colors::checkmark[0], config::colors::checkmark[1],
              config::colors::checkmark[2]);
