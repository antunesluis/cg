...
void Layer::applyBrightness(float brightness)
{
    ...
    for (int i = 0; i < imgSize; i++) {
        ...
        pixel = pixel + value;
        currentPixels[i] = static_cast<unsigned char>(clamp(pixel, 0.0f, 1.0f) * 255);
    }
}
...
void Layer::applySaturation(float saturation)
{
    ...
    for (int i = 0; i < imgSize; i += 3) {
        ...
        currentPixels[i] = static_cast<unsigned char>(clamp(r, 0.0f, 1.0f) * 255);
        currentPixels[i + 1] = static_cast<unsigned char>(clamp(g, 0.0f, 1.0f) * 255);
        currentPixels[i + 2] = static_cast<unsigned char>(clamp(b, 0.0f, 1.0f) * 255);
    }
}
...
void Layer::renderDrawnPixels()
{
    for (size_t i = 0; i < pixels.size(); i++) {
        CV::color(pixelColors[i * 3] / 255.0f, pixelColors[i * 3 + 1] / 255.0f, pixelColors[i * 3 + 2] / 255.0f);
    ...
    for (int dy = -radius; dy <= radius; dy++) {
        for (int dx = -radius; dx <= radius; dx++) {
            ...
            if (px >= 0 && px < width && py >= 0 && py < height) {
                ...
                pixelColors.insert(pixelColors.end(), {drawingColor[0], drawingColor[1], drawingColor[2]});
            }
        }
    }
}
...
void Layer::convertToGrayscale()
{
    ...
    int imgSize = imageWidth * imageHeight * 3;
    for (int i = 0; i < imgSize; i += 3) {
        unsigned char gray = static_cast<unsigned char>(0.299f * currentPixels[i] + 0.587f * currentPixels[i + 1] +
